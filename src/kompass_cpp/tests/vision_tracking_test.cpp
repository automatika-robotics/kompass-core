...
struct VisionTrackingTestConfig {
  ...
  VisionTrackingTestConfig(
      const int &maxSteps = 50, const float &time_step = 0.1,
      const float &e_pos = 0.1, const float &e_vel = 0.1,
      const float &e_acc = 0.1,
      const Eigen::Vector3f target_pose = {0.0f, 0.0f, 0.0f},
      const Eigen::Vector3f target_box_size = {0.5f, 0.5f, 1.0f},
      const int num_test_boxes = 3, const float target_v = 0.2,
      const float target_omega = 0.3)
    ...
    target_ref_vel = {target_v * cos(target_omega),
                      target_v * sin(target_omega), 0.0};
  ...
  void moveDetectedBoxes(const int step) {
    // Update the detected boxes using the velocity command with additional
    // measurement noise
    ...
    reference_path.add(step,
                       Path::Point(reference_state.x, reference_state.y, 0.0));
  };
};
...
BOOST_AUTO_TEST_CASE(test_Vision_Tracker) {
  ...
  while (step < config.maxSteps) {
    ...
    if (tracked_state) {
      ...
      LOG_INFO("Real target location at", config.reference_state.x, ", ",
               config.reference_state.y);
      LOG_INFO("Found tracked target at", mat(0), ", ", mat(1));
      tracking_error += Path::Path::distance(
          Path::Point(mat(0), mat(1), 0.0),
          Path::Point(config.reference_state.x, config.reference_state.y, 0.0));
