...
BOOST_AUTO_TEST_CASE(test_PurePursuit_All_Scenarios) {
  ...
  for (auto const &[robotName, robotType] : robotTypes) {
    for (auto const &[pathName, pathGen] : pathGenerators) {
      ...
      LOG_INFO("------------------------------------------------");
      LOG_INFO("Testing Pure Pursuit: Robot=", robotName, ", Path=", pathName);

      Control::PurePursuit controller(robotType, controlLimits, pp_config);
      ...
      while (!goal_reached && step < max_steps) {
        ...
        Control::Controller::Result result =
            controller.execute(robotState, timeStep);
        ...
        LOG_INFO("Got Control: ", result.velocity_command.vx(), ", ",
                 result.velocity_command.vy(), ", ", result.velocity_command.omega());
        step++;
      }

      if (!goal_reached) {
        LOG_WARNING(FYEL("Max steps reached without hitting goal tolerance."));
      ...
      std::string filename_base = "pure_pursuit_" + robotName + "_" + pathName;
