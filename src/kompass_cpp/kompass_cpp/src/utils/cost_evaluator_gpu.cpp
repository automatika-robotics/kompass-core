    ...
    *m_minCost = LowestCost();
    ...
    if ((costWeights.getParameter<double>("reference_path_distance_weight") >
      ...
      if ((weight = costWeights.getParameter<double>("goal_distance_weight") >
                    0.0)) {
        goalCostFunc(trajs.size(), reference_path.points.size(),
...
void CostEvaluator::pathCostFunc(const size_t trajs_size,
  ...
  m_q.submit([&](sycl::handler &h) {
    ...
    sycl::accessor<double, 1, sycl::access::mode::read_write,
        ...
        pointCost(sycl::range<1>(pathSize), h);
    // local memory for storing per trajectory average cost
    sycl::accessor<double, 1, sycl::access::mode::read_write,
                   sycl::access::target::local>
        trajCost(sycl::range<1>(trajs_size), h);
...
void CostEvaluator::goalCostFunc(const size_t trajs_size,
  ...
  m_q.submit([&](sycl::handler &h) {
    ...
    // Kernel scope
    m_q.parallel_for(sycl::range<1>(global_size), [=](sycl::id<1> id) {
      sycl::vec<double, 2> last_path_point = {X[id * pathSize + pathSize - 1],
                                              Y[id * pathSize + pathSize - 1]};
      sycl::vec<double, 2> last_ref_point = {ref_X[refPathSize - 1],
                                             ref_Y[refPathSize - 1]};
      // end point distance normalized by path length
      double distance =
          sycl::distance(last_path_point, last_ref_point) / pathLength;
