...
void RGBFollower::getFindTargetCmds(const int last_direction) {
  ...
  search_commands_queue_ = std::queue<Eigen::Vector3d>();
...
void RGBFollower::trackTarget(const Bbox2D &target) {
  ...
  float error_y = 2.0f * (static_cast<float>(target.getCenter().y()) /
                              static_cast<float>(target.img_size.y()) -
                          0.5f);
  float error_x = 2.0f * (static_cast<float>(target.getCenter().x()) /
                              static_cast<float>(target.img_size.x()) -
                          0.5f);
  ...
  LOG_DEBUG("Img error x: ", error_x,
            ", center_x: ", static_cast<float>(target.getCenter().x()),
            ", img_size_x: ", static_cast<float>(target.img_size.x()));
  LOG_DEBUG("Img error y: ", error_y,
            ", center_y: ", static_cast<float>(target.getCenter().y()),
            ", img_size_y: ", static_cast<float>(target.img_size.y()));
  ...
  if (std::abs(dist_error_) < distance_tolerance &&
    ...
    out_vel_.add(0, 0.0, 0.0, 0.0);
    return;
  }

  dist_speed = std::abs(dist_error_) > distance_tolerance
                   ? (dist_error_ / config_.target_distance()) *
